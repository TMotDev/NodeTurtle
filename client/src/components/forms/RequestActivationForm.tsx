...
export default function RequestActivationForm() {
  ...
  async function onSubmit(values: z.infer<typeof passwordResetSchema>) {
    ...
    const result = await API.post("/auth/activate", {
      email: values.email,
    });
    ...
    <>
      {formStatus.success ? (
        <Alert variant="success">
          ...
          <AlertDescription>
            Account activation link sent! Please check your inbox to activate your account.
          </AlertDescription>
        </Alert>
      ) : (
        <>
